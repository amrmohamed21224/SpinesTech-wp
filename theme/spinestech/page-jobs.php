<?php
/**
 * Used for page slug "jobs" (child of careers â†’ /careers/jobs/)
 */
get_header();
...
<main class="pt-24 sm:pt-28 lg:pt-32 pb-24 px-margin-mobile md:px-margin-desktop text-start">
    <div class="max-w-container-max mx-auto">
        <a href="<?php echo esc_url(st_url('/careers/')); ?>" class="inline-flex items-center gap-2 text-secondary font-bold mb-8"><span class="material-symbols-outlined" aria-hidden="true">arrow_back</span><?php echo $locale === 'ar' ? 'Ø§Ù„Ø¹ÙˆØ¯Ø© Ù„Ù„ØªÙˆØ¸ÙŠÙ' : 'Back to careers'; ?></a>
        ...
        <form id="career-form" data-st-career-form enctype="multipart/form-data" class="max-w-2xl p-8 bg-surface-container-low rounded-2xl border border-outline-variant/20 space-y-4">
            ...
            <select name="position" class="w-full px-4 py-3 rounded-xl border border-outline-variant/40 bg-white outline-none">
                <option value=""><?php echo $locale === 'ar' ? 'Ø§Ø®ØªØ± Ø§Ù„ÙˆØ¸ÙŠÙØ©' : 'Select position'; ?></option>
            ...
            <textarea name="coverLetter" rows="4" placeholder="<?php echo $locale === 'ar' ? 'Ø±Ø³Ø§Ù„Ø© ØªØ¹Ø±ÙŠÙÙŠØ©' : 'Cover letter'; ?>" class="w-full px-4 py-3 rounded-xl border border-outline-variant/40 bg-white outline-none resize-y"></textarea>
