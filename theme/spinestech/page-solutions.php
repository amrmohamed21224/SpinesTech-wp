...
$locale = st_locale();
$needs = [
    'software' => $locale === 'ar' ? 'Ù…Ù†ØµØ© Ø£Ùˆ ØªØ·Ø¨ÙŠÙ‚' : 'Custom app',
    'ai' => $locale === 'ar' ? 'Ø°ÙƒØ§Ø¡ Ø§ØµØ·Ù†Ø§Ø¹ÙŠ' : 'AI & automation',
    'operations' => $locale === 'ar' ? 'ERP / CRM' : 'ERP / CRM',
    'commerce' => $locale === 'ar' ? 'ØªØ¬Ø§Ø±Ø© Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠØ©' : 'E-commerce',
];
?>
<main class="pt-24 sm:pt-28 lg:pt-32 pb-24 px-margin-mobile md:px-margin-desktop text-start" id="st-solutions-page">
    ...
    <div class="max-w-container-max mx-auto grid lg:grid-cols-12 gap-8">
        <aside class="lg:col-span-4 space-y-8">
            <div>
                <h2 class="text-headline-sm font-bold text-primary mb-4"><?php echo $locale === 'ar' ? 'Ù…Ø§ Ø§Ù„Ø°ÙŠ ØªØ¨Ø­Ø« Ø¹Ù†Ù‡ØŸ' : 'What do you need?'; ?></h2>
            ...
            <?php if ($sectors) : ?>
                <div>
                    <h2 class="text-headline-sm font-bold text-primary mb-4"><?php echo $locale === 'ar' ? 'Ø§Ù„Ù‚Ø·Ø§Ø¹' : 'Sector'; ?></h2>
                    <div class="flex flex-wrap gap-2" data-st-solution-sectors>
                        <button type="button" data-sector="" class="st-sector-chip px-3 py-1.5 rounded-full border border-secondary bg-secondary/10 text-sm font-bold"><?php echo $locale === 'ar' ? 'Ø§Ù„ÙƒÙ„' : 'All'; ?></button>
        ...
        <div class="lg:col-span-8">
            <h2 class="text-headline-sm font-bold text-primary mb-6"><?php echo $locale === 'ar' ? 'Ø§Ù„ØªØ±Ø´ÙŠØ­Ø§Øª' : 'Recommendations'; ?></h2>
